...
# Read the endpoint from tedge
c8y_url = subprocess.check_output(["tedge", "config", "get", "c8y.url"], universal_newlines=True).strip()

client.publish('c8y/s/us','501,c8y_RemoteAccessConnect')
device_proxy = DeviceProxy(host, int(port), None, connectionKey, c8y_url, None, None, jwt, on_close_handler)
